...
static void push_collector(lua_State* L, const char* type, lua_CFunction col) {
    ...
    /* pushStack collector function, but only if it's not NULL, or if there's no
       collector already */
    if (!col) return;
...
{
    char comame[128] = "const ";
    char cbase[128] = "const ";
    strncat(comame,name,120);
    strncat(cbase,base,120);

    mapinheritance(L,name,base);
    mapinheritance(L,comame,name);

    mapsuper(L,comame,cbase);
...
//---- by SunLightJuly, 2014.6.5
    ...
    push_collector(L, comame, col);
    /*
    luaL_getmetatable(L,comame);
    lua_pushstring(L,".collector");
    lua_pushcfunction(L,col);
    lua_rawset(L,-3);
    lua_pop(L,1);
